var Thread = {
		...
		if (quoted) {
			...
			post_ids.each(function(post_id) {
				if ($("multiquote_" + post_id)) {
					...
					element.src = element.src.replace("postbit_multiquote.gif",
	...
	multiQuote : function(pid) {
		...
		if (is_new == true) {
			...
			element.src = element.src.replace("postbit_multiquote_on.gif",
	...
	loadMultiQuoted : function() {
		if (use_xmlhttprequest == 1) {
			...
			new Ajax.Request(zing_forum_url
	...
	deletePost : function(pid) {
		...
		if (confirmReturn == true) {
			...
			// "editpost.php?action=deletepost&delete=1");
			form.setAttribute("action", zing_forum_index
					+ "zforum=editpost&action=deletepost&delete=1");
			form.setAttribute("style", "display: none;");

			if (my_post_key) {
				if (MyBB.browser == "ie") {
					var input = document
							.createElement("<input name=\"my_post_key\">");
				} else {
	...
	reportPost : function(pid) {
		MyBB.popupWindow(zing_forum_url_ajax("report.php?pid=" + pid),
				"reportPost", 400, 300)
	},
	...
	quickEdit : function(pid) {
		...
		new Ajax.Request(zing_forum_url
			...
			method : 'get',
			onComplete : function(request) {
				Thread.quickEditLoaded(request, pid);
			}
		});
		return false;
	},
	...
	quickEditSave : function(pid) {
		...
		new Ajax.Request(zing_forum_url
				+ 'xmlhttp.php?action=edit_post&do=update_post&pid=' + pid
				+ "&my_post_key=" + my_post_key, {
			method : 'post',
			postBody : postData,
			onComplete : function(request) {
				Thread.quickEditSaved(request, pid);
			}
		});
	},
	...
	initQuickReply : function() {
		if ($('quick_reply_form') && use_xmlhttprequest == 1) {
			Event.observe($('quick_reply_submit'), "click", Thread.quickReply
	...
	quickReply : function(e) {
		...
		new Ajax.Request(zing_forum_url + 'newreply.php?ajax=1', {
			method : 'post',
			postBody : post_body,
			onComplete : function(request) {
				Thread.quickReplyDone(request);
			}
		});
		return false;
	},
	...
	quickReplyDone : function(request) {
		if ($('captcha_img')) {
			captcha = request.responseText
					.match(/^<captcha>([0-9a-zA-Z]+)(\|([0-9a-zA-Z]+)|)<\/captcha>/);
			if (captcha) {
				...
				if (captcha[1]) {
					...
					if (captcha[3]) {
						...
						$('captcha_img').src = "captcha.php?action=regimage&imagehash="
								+ imghash;
		...
		if (request.responseText.match(/<error>([^<]*)<\/error>/)) {
			...
			if (MyBB.browser == "ie" || MyBB.browser == "opera"
					|| MyBB.browser == "safari" || MyBB.browser == "chrome") {
			...
			scripts = scripts.replace('.php?', '&');
			scripts = scripts.replace('window.location = "',
					'window.location = "' + zing_forum_index + 'zforum=');
			// alert(scripts);
